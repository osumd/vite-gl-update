...
function InstancedCylinder({bottom, top, divisions})
{
        ...
        let vertices = new Float32Array(4*(4*3*12));
        let normals = new Float32Array(4*(4*3*12));
        let uvs = new Float32Array(4*(2*4*12));
        let elements = new Uint16Array(4*(6*12));

        let currentVertex = 0;
        let j = 0;
        let push_array = [0,0];

        let difference = top.clone().sub(bottom);



        var uux = new THREE.Vector3(difference.x, difference.y, 0);
        var uuz = new THREE.Vector3(0,difference.y,difference.z);

        push_array = push_cylinder_back(start, end, 0.1, vertices, normals,uvs,elements, currentVertex, j, 12)
        currentVertex = push_array[0]; j = push_array[1];

        // push_array = push_cylinder_back(start, uux, 0.1, vertices, normals,uvs,elements, currentVertex, j, 12)
        // currentVertex = push_array[0]; j = push_array[1];
        ...
        var instanceCount = (2*divisions)+2;

        useEffect(()=>{
            if (meshRef.current) {

              let ux = new THREE.Vector3(difference.x,0,0);
              let dxx = (ux.length()/divisions);
              let uy = new THREE.Vector3(0,difference.y,difference.z);
              let dyy = (uy.length()/divisions);
              ...
              let uyl = uy.length(); 
               ...
               let c = ux.clone().cross(new THREE.Vector3(0,1,0)).normalize();
              //  push_array = push_cylinder_back(start, c, 0.1, vertices, normals,uvs,elements, currentVertex, j, 12)
              //  currentVertex = push_array[0]; j = push_array[1];
               
               let c1 = uy.clone().cross(new THREE.Vector3(0,1,0)).normalize();

               let d = new THREE.Vector3(0,1,0).dot(ux);
               let d1 = new THREE.Vector3(0,1,0).dot(uy);
               
               let a = Math.acos(d/ux.length());
               let a1 = Math.acos(d1/uy.length());

               ux.normalize();
               uy.normalize();

              for (let i = 0; i < divisions+1; i++) {
                let position = new THREE.Vector3(bottom.x+(ux.x*dxx*i),bottom.y+(ux.y*dxx*i), bottom.z);
                let quaternion = new THREE.Quaternion().setFromAxisAngle(c1,-a1);

                meshRef.current.setMatrixAt(i, new THREE.Matrix4().compose(position, quaternion, new THREE.Vector3(1, uyl, 1)));
              }

              for (let i = 0; i < divisions+1; i++) {
                let position = new THREE.Vector3(bottom.x,bottom.y+(uy.y*dyy*i), bottom.z+(uy.z*dyy*i));
                let quaternion = new THREE.Quaternion().setFromAxisAngle(c,-a);

                meshRef.current.setMatrixAt(i+divisions+1, new THREE.Matrix4().compose(position, quaternion, new THREE.Vector3(1, uxl, 1)));
              }

              // for (let i = 0; i < 1; i++) {
              //   let position = new THREE.Vector3(bottom.x,bottom.y, bottom.z);
              //   let quaternion = new THREE.Quaternion().setFromAxisAngle(c,-a);

              //   meshRef.current.setMatrixAt(i, new THREE.Matrix4().compose(position, quaternion, new THREE.Vector3(1, 1, 1)));
              // }
              ...
              //     quaternion.setFromAxisAngle(c1, a1);
              //     meshRef.current.setMatrixAt(i+divisions, new THREE.Matrix4().compose(position, quaternion, new THREE.Vector3(1, 1, 1)));
                
              // }

                meshRef.current.instanceMatrix.needsUpdate = true;
              }
        },[bottom,top,divisions,difference]);

        return (
            <instancedMesh ref={meshRef} args={[bufferGeometry, new THREE.MeshBasicMaterial({ color: 0xff0000, side: THREE.DoubleSide }), instanceCount]}>
            </instancedMesh>
          );
...
function InstancedCylinderAdv({bottom, top, xdivisions, ydivisions})
{
        //console.log(bottom,top,divisions);
        ...
        push_array = push_cylinder_back(start, end, (difference.x/xdivisions)/10, vertices, normals,uvs,elements, currentVertex, j, 12)
        currentVertex = push_array[0]; j = push_array[1];
        ...
        var instanceCount = (2*(xdivisions+ydivisions))+2;

        useEffect(()=>{
            if (meshRef.current) {

              let ux = new THREE.Vector3(difference.x,0,0);
              let dxx = (ux.length()/xdivisions);
              let uy = new THREE.Vector3(0,difference.y,difference.z);
              let dyy = (uy.length()/xdivisions);
              ...
              for (let i = 0; i < ydivisions+1; i++) {
                ...
                meshRef.current.setMatrixAt(i+xdivisions+1, new THREE.Matrix4().compose(position, quaternion, new THREE.Vector3(1, uxl, 1)));
              }
        ...
        },[bottom,top,xdivisions,ydivisions,difference]);
...
export {InstancedCylinder,InstancedCylinderAdv};
